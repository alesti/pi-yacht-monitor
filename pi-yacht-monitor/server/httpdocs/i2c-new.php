...
require_once("./i2c/PCF8574_OUT.php");
require_once("./i2c/PCF8574_OUT_INV.php");
require_once("./i2c/HORTER_PIC18F13K22_AD.php");
...
if  ($step == 1) {
  ...
  echo '<option value="PCF8574_OUT">Digital output PCF8574</option>';
  echo '<option value="PCF8574_OUT_INV">Digital output PCF8574 invers</option>';
  echo '<option value="HORTER_PIC18F13K22_AD">Horter 5ch 10bit A/D</option>';
  echo '</select></td></tr>';
  ...
  if ($type == "PCF8591") {
    ...
    if (!$redis->exists("config.i2c.$name")) {
      ...
      $mod->type=$type;
      echo $mod->getNewForm("?step=3");
  } else if ($type == "PCF8574_OUT_INV") {
      $mod = new PCF8574_OUT_INV;
      $mod->bus = $bus;
      $mod->address = $address;
      $mod->active = $active;
      $mod->name=$name;
      $mod->type=$type;
      echo $mod->getNewForm("?step=3");
  } else if ($type == "HORTER_PIC18F13K22_AD") {
      $mod = new HORTER_PIC18F13K22_AD;
      $mod->bus = $bus;
      $mod->address = $address;
      $mod->active = $active;
      $mod->name=$name;
  ...
  if ($type == "PCF8591") {
    ...
    $in2active = isset($_POST["in-2-active"])?1:0; $in2name   = $_POST["in-2-name"]; $in2factor = $_POST["in-2-factor"];
    $in3active = isset($_POST["in-3-active"])?1:0; $in3name   = $_POST["in-3-name"]; $in3factor = $_POST["in-3-factor"];
    $in4active = isset($_POST["in-4-active"])?1:0; $in4name   = $_POST["in-4-name"]; $in4factor = $_POST["in-4-factor"];
    $address = $_POST["address"];
    $bus = $_POST["bus"];
    $active = $_POST["active"];
    $name = $_POST["name"];
    if (!$redis->exists("config.i2c.$name")) {
      ...
      $redis->hset($key,"in2-active",$in2active); $redis->hset($key,"in2-name",$in2name); $redis->hset($key,"in2-factor",$in2factor);
      $redis->hset($key,"in3-active",$in3active); $redis->hset($key,"in3-name",$in3name); $redis->hset($key,"in3-factor",$in3factor);
      $redis->hset($key,"in4-active",$in3active); $redis->hset($key,"in4-name",$in4name); $redis->hset($key,"in4-factor",$in4factor);
      echo "Erfolgreich gespeichert";
    } else {
      echo "Nicht gespeichert, Name existiert schon!";
    }
