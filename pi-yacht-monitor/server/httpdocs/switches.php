...
echo "<tr><th>Name</th><th>Output Typ</th><th>Port</th><th>Name</th><th>Istwert</th><th>Aktion</th></tr>";
foreach ($keys as $key) {
  $v = $redis->hgetall($key);
  $type = $v["type"];
  $name = explode(".",$key)[2];
  if ($type == "PCF8574_OUT_INV") {
    for ($i = 0; $i < 8; $i++) {
       ...
       echo "<td>$value</td>";
       echo "<td><input type='submit' name='switch' value='$text'/>";
       echo "<input type='hidden' name='key' value='$key'/>";
       echo "<input type='hidden' name='subkey' value='$subkey'/>";
       echo "<input type='hidden' name='givenvalue' value='$givenvalue'/>";
       echo "</td>";
       echo "</tr>";
       echo "</form>";
    }
